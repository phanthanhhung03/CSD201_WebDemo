...
export default function QueueCanvas({ snapshot }) {
  const { nodes = [], frontId, rearId, size = 0, capacity = 6, alertMessage } = snapshot || {};
  ...
  return (
    <div className="relative w-full h-[380px] bg-dark-bg/60 rounded-xl overflow-hidden border border-dark-border shadow-inner">
      ...
      {/* Queue Lane Markers */}
      <div className="absolute left-4 top-12 flex items-center gap-2 text-rose-400 font-bold text-xs bg-rose-950/60 px-3 py-1 rounded-lg border border-rose-800/60 z-20">
        &larr; DEQUEUE (FRONT)
      </div>

      <div className="absolute right-4 top-12 flex items-center gap-2 text-emerald-400 font-bold text-xs bg-emerald-950/60 px-3 py-1 rounded-lg border border-emerald-800/60 z-20">
        ENQUEUE (REAR) &rarr;
      </div>
