...
export default function BSTCanvas({ snapshot }) {
  const { nodes = [], pointers = {}, rootId } = snapshot || {};
  ...
  return (
    <div className="relative w-full h-[400px] bg-dark-bg/60 rounded-xl overflow-hidden border border-dark-border flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:16px_16px] opacity-40 pointer-events-none" />

      {/* SVG Layer for Tree Branches (Edges) */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
        ...
        {rootId && nodeMap.has(rootId) && (
          <SVGArrow
            ...
            startY={nodeMap.get(rootId).y - 45}
            endX={nodeMap.get(rootId).x}
            endY={nodeMap.get(rootId).y - 28}
            color="#8b5cf6"
            label="ROOT"
          />
        )}
        ...
        <AnimatePresence mode="popLayout">
          {nodes.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="w-full text-center text-slate-500 text-sm font-mono py-28"
            >
              [ BST Rỗng - root = NULL ]
            </motion.div>
          ) : (
            nodes.map((node) => {
              ...
              const statusStyles = {
                ...
                active: 'border-amber-400 bg-amber-950 text-amber-200 glow-amber scale-110',
                found: 'border-emerald-400 bg-emerald-950 text-emerald-200 glow-emerald scale-110',
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-center justify-center rounded-full border-2 shadow-xl font-bold font-mono text-sm transition-all ${
